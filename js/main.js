...
ui.btnFinishRoute.onclick = async () => {
    const name = prompt("Deseja dar um nome a esta trilha?", state.currentRoute.name);
    ...
    container.innerHTML = captured.map(n => {
        ...
        return `
            <div class="card">
                ...
                <div style="color:${faltam <= 0 ? '#ef4444' : '#22c55e'}; font-weight:bold; margin-top:5px">
                    ${faltam <= 0 ? "⚠️ RETIRAR JÁ!" : "⏳ Faltam " + faltam + " dias"}
                </div>
            </div>
        `;
    }).join("");
