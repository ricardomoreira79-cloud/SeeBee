...
async function handleNestUpdate(routeId, nestId, newStatus, newNotes, newSpecies) {
    ...
    if (newStatus === 'capturado') {
        ...
        delete nest.species; // Remove a propriedade se não for mais capturado
    }
...
function bindUI() {
  ...
  ui.btnToggleRoute.addEventListener("click", async () => {
    if (!state.currentRoute) startRoute(); else await stopRoute(); renderRoutesList();
  });
  ui.btnAddNest.addEventListener("click", handleAddNest);
  ui.btnRefresh.addEventListener("click", async () => { await refreshData(); });
}
